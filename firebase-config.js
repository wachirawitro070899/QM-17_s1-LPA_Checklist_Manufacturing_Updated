...
(function(){
  function boot(){
    ...
    const pf=document.createElement('div');pf.className='field';pf.id='productionField';pf.innerHTML='<label>Production</label><input id="production" type="text" placeholder="กรอก Production">';
    ...
    const workstation=document.getElementById('workstation'),production=document.getElementById('production');production.removeAttribute('readonly');production.readOnly=false;production.disabled=false;const fixed={5:'U375 / Thinbride / APB-Mi Op10',6:'T6 / PBR / P2-30A / APB-Mi Op20',8:'U375 / P2-30A / Thin bride / APB-Mi'};
    function syncFields(){const area=areaNumber(line.value);const clean=String(line.value||'').replace(/^Area\s+\d+\s*/i,'').trim();const value=fixed[area]||clean;workstation.value=value;production.removeAttribute('readonly');production.readOnly=false;production.disabled=false;syncAuditee();}
